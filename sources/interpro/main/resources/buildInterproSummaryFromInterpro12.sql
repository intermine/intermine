drop index idx_e2m_method_ac;
drop index idx_e2m_entry_ac;

drop table common_annotation;
create table common_annotation as select common_annotation_id, ann_id, name, text, comments from ##INTERPRO_SRC_ALIAS##.common_annotation;

drop table cv_entry_type;
create table cv_entry_type as select cv_entry_type_id, code, abbrev, description from ##INTERPRO_SRC_ALIAS##.cv_entry_type;

drop table cv_relation;
create table cv_relation as select cv_relation_id, code, abbrev, description, forward, reverse from ##INTERPRO_SRC_ALIAS##.cv_relation;

drop table cv_evidence;
create table cv_evidence as select cv_evidence_id, code, abbrev, description from ##INTERPRO_SRC_ALIAS##.cv_evidence;

drop table cv_database;
create table cv_database as select cv_database_id, dbcode, dbname, dborder, dbshort from ##INTERPRO_SRC_ALIAS##.cv_database;

drop table db_version;
create table db_version as select db_version_id, dbcode, version, entry_count, file_date, load_date from ##INTERPRO_SRC_ALIAS##.db_version where dbcode in (select dbcode from cv_database);
...
insert into taxonomy (taxa_id)  select distinct tax_id from ##INTERPRO_SRC_ALIAS##.taxonomy2protein where tax_id in (##ORGANISM_TAXID_LIST##);

drop table protein2taxonomy;
create table protein2taxonomy as select taxonomy2protein_id, protein_ac, tax_id, hierarchy, tax_name_concat from ##INTERPRO_SRC_ALIAS##.taxonomy2protein where tax_id in (select taxa_id from taxonomy);
...
drop table protein;
create table protein as select p.protein_id, p.protein_ac, p.name, p.dbcode, p.crc64, p.len, p.fragment, p.struct_flag from ##INTERPRO_SRC_ALIAS##.protein p, protein2taxonomy p2t where p.protein_ac = p2t.protein_ac;
...
drop table method;
create table method as select method_id, method_ac, name, dbcode, method_date, skip_flag from ##INTERPRO_SRC_ALIAS##.method;
...
drop table entry;
create table entry as select entry_id, entry_ac, entry_type, name, checked, created, short_name from ##INTERPRO_SRC_ALIAS##.entry;
...
drop table supermatch;
create table supermatch as select s.supermatch_id, s.protein_ac, s.entry_ac, s.pos_from, s.pos_to from ##INTERPRO_SRC_ALIAS##.supermatch s, protein p where s.protein_ac = p.protein_ac;
...
drop table matches;
create table matches as select m.matches_id, m.protein_ac, m.method_ac, m.pos_from, m.pos_to, m.status, m.dbcode, m.evidence, m.seq_date, m.match_date, m.score from ##INTERPRO_SRC_ALIAS##.matches m, protein p where m.protein_ac = p.protein_ac and m.pos_from is not null and m.pos_to is not null;
...
drop table entry2common_annotation;
create table entry2common_annotation as select entry2common_id as entry2common_annotation_id, entry_ac, ann_id, order_in from ##INTERPRO_SRC_ALIAS##.entry2common;
...
drop table entry2cv_database;
create table entry2cv_database as select entry_xref_id as entry2cv_database_id, entry_ac, dbcode, ac, name from ##INTERPRO_SRC_ALIAS##.entry_xref;
...
drop table entry2method;
create table entry2method as select entry2method_id, entry_ac, method_ac, evidence, ida from ##INTERPRO_SRC_ALIAS##.entry2method;
...
drop table entry2comp;
create table entry2comp as select entry2comp_id, entry1_ac, entry2_ac, relation from ##INTERPRO_SRC_ALIAS##.entry2comp;
...
drop table entry2entry;
create table entry2entry as select entry2entry_id, entry_ac, parent_ac, relation from ##INTERPRO_SRC_ALIAS##.entry2entry;
