...
<div id="gBrowse" class="feature basic-table">
  ...
  <c:set var="label0" value="MGI_Representative_Transcripts" />
  <c:set var="label1" value="MGI_Representative_Transcripts-ENSEMBL_Transcripts-NCBI_Transcripts-VEGA_Transcripts" />

  <c:choose>
  <c:when test="${WEB_PROPERTIES['gbrowse.database.source'] != null}">
    <div class="loading">
      <html:link href="${WEB_PROPERTIES['gbrowse.prefix']}/${WEB_PROPERTIES['gbrowse.database.source']}?source=${WEB_PROPERTIES['gbrowse.database.source']};start=${loc.start};stop=${loc.end};ref=${loc.locatedOn.primaryIdentifier};label=${label1};grid=on"></html:link>
    </div>
	<script type="text/javascript">
	  jQuery(document).ready(function() {
	      ...
	      jQuery(img)
	        ...
	        .attr('src', "${WEB_PROPERTIES['gbrowse_image.prefix']}/${WEB_PROPERTIES['gbrowse.database.source']}?source=${WEB_PROPERTIES['gbrowse.database.source']};label=${label0};name=${name};width=600;b=1")
