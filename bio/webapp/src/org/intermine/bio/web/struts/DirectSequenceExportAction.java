 ...
 */
public class DirectSequenceExportAction extends InterMineAction
{
    @SuppressWarnings("unused")
    private static final Logger LOG = Logger.getLogger(DirectSequenceExportAction.class);
    ...
    public ActionForward execute(ActionMapping mapping,
      ...
      String sequenceHeader = request.getParameter("header");
      if (sequenceHeader == null || sequenceHeader.isEmpty()) {
        sequenceHeader = "Untitled Sequence";
      }
      Sequence obj = (Sequence)os.getObjectById(objectId);

      response.setContentType("text/plain");
      PrintWriter out = response.getWriter();
      out.write(">"+sequenceHeader+"\n");
      String residues = new String(obj.getResidues().toString());
      for(int ctr=0; ctr < residues.length(); ctr+=60){
        int end_write = residues.length() > ctr+60?ctr+60:residues.length();
        out.write(residues.substring(ctr,end_write)+"\n");
      }
      out.flush();
...
/*
    ...
    private BioSequence createBioSequence(InterMineObject obj)
        ...
        Protein bioEntity = new Protein();
        bioEntity.setPrimaryIdentifier("Consensus Sequence");
        Seqence s = new Sequence();
        bioEntity.setSequence(s);
        bioSequence = BioSequenceFactory.make(bioEntity, SequenceType.RNA);
        if (bioSequence == null) {
            return null;
        }
