...
ant -f $DIR/webapp/build.xml remove-webapp >> $DIR/log/remove-webapp.log

# Allow callers to skip the DB build and go straight to the web-app
if [ -z "$SKIP_BUILD" ]
then
...
ant release-webapp | tee -a $DIR/log/build-webapp.log | grep tomcat-deploy
