...
public class TruncatedSqlGeneratorTest extends SqlGeneratorTest
{
    ...
    public static void setUpResults() throws Exception {
        results.put("SelectSimpleObject", "SELECT Company.OBJECT AS \"Company\", Company.id AS \"Companyid\" FROM InterMineObject AS Company WHERE Company.class = 'org.intermine.model.testmodel.Company' ORDER BY Company.id");
        results.put("SubQuery", "SELECT DISTINCT a1_.a1_name AS a2_, a1_.Alias AS a3_ FROM (SELECT DISTINCT a1_.OBJECT AS a1_, a1_.CEOId AS a1_CEOId, a1_.addressId AS a1_addressId, a1_.id AS a1_id, a1_.name AS a1_name, a1_.vatNumber AS a1_vatNumber, 5 AS Alias FROM InterMineObject AS a1_ WHERE a1_.class = 'org.intermine.model.testmodel.Company') AS a1_ ORDER BY a1_.a1_name, a1_.Alias");
