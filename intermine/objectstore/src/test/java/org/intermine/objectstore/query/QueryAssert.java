...
public class QueryAssert {
    public static void assertEquals(Query q1, Query q2) {
    ...
    private static void checkQueryClassLists(String msg, Collection l1, Collection l2, Query q1, Query q2) {
        Assert.assertEquals(msg + ": lists are different sizes", l1.size(), l2.size());
    ...
    private static void checkObjects(String msg, Object qc1, Object qc2, Query q1, Query q2) {
        ...
        if (qc1 instanceof QueryNode) {
            if (qc2 instanceof QueryNode) {
                ...
                Assert.fail(msg + ": QueryNode does not match " + qc2.getClass().getName());
            }
        } else if (qc1 instanceof Query) {
            if (qc2 instanceof Query) {
                assertEquals(msg + ": subquery", (Query) qc1, (Query) qc2);
            } else {
                Assert.fail(msg + ": Subquery does not match " + qc2.getClass().getName());
            }
            ...
            if (qc2 instanceof QueryClassBag) {
                ...
                Assert.fail(msg + ": QueryClassBag does not match  " + qc2.getClass().getName());
            }
            ...
            if (qc2 instanceof PathExpressionField) {
                ...
                Assert.assertEquals(msg + ": PathExpressionField column numbers are not equal", pef1.getFieldNumber(), pef2.getFieldNumber());
            }
            ...
            if (qc2 instanceof QueryObjectPathExpression) {
                ...
                Assert.assertEquals(msg + ": QueryObjectPathExpression fieldnames are not equal", pe1.getFieldName(), pe2.getFieldName());
                ...
                Assert.fail(msg + ": QueryObjectPathExpression does not match " + qc2.getClass().getName());
            }
            ...
            if (qc2 instanceof QueryCollectionPathExpression) {
                ...
                Assert.assertEquals(msg + ": QueryCollectionPathExpression fieldNames are not equal", pe1.getFieldName(), pe2.getFieldName());
                ...
                Assert.assertEquals(msg + ": QueryCollectionPathExpression isSingleton is not equal", pe1.isSingleton(), pe2.isSingleton());
            } else {
                Assert.fail(msg + ": QueryCollectionPathExpression does not match " + qc2.getClass().getName());
            }
            ...
            if (qc2 instanceof ObjectStoreBag) {
                if (((ObjectStoreBag) qc1).getBagId() != ((ObjectStoreBag) qc2).getBagId()) {
                    Assert.fail(msg + ": ObjectStoreBag ids are not equal");
                }
            } else {
                Assert.fail(msg + ": ObjectStoreBag does not match " + qc2.getClass().getName());
            }
            ...
            if (qc2 instanceof OrderDescending) {
                ...
                Assert.fail(msg + ": OrderDescending does not match " + qc2.getClass().getName());
            }
            ...
            if (qc2 instanceof QueryReference) {
                ...
                Assert.fail(msg + ": QueryReference does not match " + qc2.getClass().getName());
            }
        } else if (qc1 instanceof ObjectStoreBagCombination) {
            if (qc2 instanceof ObjectStoreBagCombination) {
                if (!qc1.equals(qc2)) {
                    Assert.fail(msg + ": ObjectStoreBagCombinations are not equal");
                }
            } else {
                Assert.fail(msg + ": ObjectStoreBagCombination does not match " + qc2.getClass().getName());
            }
        } else if (qc1 instanceof ObjectStoreBagsForObject) {
            if (qc2 instanceof ObjectStoreBagsForObject) {
                if (!qc1.equals(qc2)) {
                    Assert.fail(msg + ": ObjectStoreBagsForObjects are not equal");
                }
            } else {
                Assert.fail(msg + ": ObjectStoreBagsForObject does not match " + qc2.getClass().getName());
            }
        } else {
            Assert.fail(msg + ": Unknown type of Object in list: " + qc1.getClass().getName());
        }
    }

    private static void checkQueryNodes(String msg, Object qn1, Object qn2, Query q1, Query q2) {
        if ((qn1 == null) && (qn2 == null)) {
            return;
        }

        Assert.assertNotNull(msg + ": first QueryNode is null, second isn't", qn1);
        Assert.assertNotNull(msg + ": first QueryNode is not null, second is", qn2);

        Assert.assertEquals(msg + ": QueryNodes are not the same class", qn1.getClass(), qn2.getClass());

        if (qn1 instanceof QueryClass) {
            QueryClass qc1 = (QueryClass) qn1;
            QueryClass qc2 = (QueryClass) qn2;

            Assert.assertEquals(msg + ": QueryClasses do not refer to the same class", qc1.getType(), qc2.getType());
            Assert.assertEquals(msg + ": QueryClasses do not have the same alias", q1.getAliases().get(qc1),
            ...
            if (fe1 instanceof QueryClass) {
                if (fe2 instanceof QueryClass) {
                    checkQueryNodes(msg, fe1, fe2, q1, q2);
                } else {
                    Assert.fail(msg + ": field member of QueryClass does not match field member of subquery");
                }
            } else {
                if (fe2 instanceof QueryClass) {
                    Assert.fail(msg + ": field member of subquery does not match field member of QueryClass");
                } else {
                    String alias1 = q1.getAliases().get(fe1);
                    String alias2 = q2.getAliases().get(fe2);
                    Assert.assertNotNull(msg + ": alias1 is null - " + q1.getAliases() + ", " + fe1, alias1);
                    Assert.assertNotNull(msg + ": alias2 is null - " + q2.getAliases() + ", " + fe2, alias2);
                    Assert.assertEquals(msg + ": field members of different subquery aliases", q1.getAliases().get(fe1), q2.getAliases().get(fe2));
                }
            }
            Assert.assertEquals(msg + ": QueryField fieldnames are not equal", qf1.getFieldName(), qf2.getFieldName());
        } else if (qn1 instanceof QueryValue) {
            QueryValue qv1 = (QueryValue) qn1;
            QueryValue qv2 = (QueryValue) qn2;

            Assert.assertEquals(msg + ": QueryValues are not equal", qv1, qv2);
            ...
            Assert.assertEquals(msg + ": functions are not the same", qf1.getOperation(), qf2.getOperation());
        } else if (qn1 instanceof QueryExpression) {
            QueryExpression qe1 = (QueryExpression) qn1;
            QueryExpression qe2 = (QueryExpression) qn2;

            Assert.assertEquals(msg + ": type of QueryExpressions are not the same", qe1.getType(), qe2.getType());
    ...
    private static void checkConstraints(String msg, Constraint c1, Constraint c2, Query q1, Query q2) {
        if ((c1 == null) && (c2 == null)) {
            return;
        }

        Assert.assertNotNull(msg + ": first Constraint is null, second isn't", c1);
        Assert.assertNotNull(msg + ": first Constraint is not null, second is", c2);

        // Check that the Constraints are the same class
        Assert.assertEquals(msg + ": Constraints are not the same class", c1.getClass(), c2.getClass());
        Assert.assertEquals(msg + ": Constraints don't have the same op", c1.getOp(), c2.getOp());
        ...
        if (c1 instanceof ConstraintSet) {
            ...
            Assert.assertEquals(msg + ": ConstraintSets are not the same size",
            ...
            if (cc1.getArg2QueryClass() != null) {
                ...
                Assert.assertEquals(msg + ": second objects in ClassConstraint not equal",
            ...
            Assert.assertEquals(msg + ": Bags are not equal", cc1.getBag(), cc2.getBag());
            Assert.assertEquals(msg + ": OSBs are not equal", cc1.getOsb(), cc2.getOsb());
            ...
            Assert.assertEquals(msg + ": evaluable list sizes are not the same", bc1.getEvaluables().size(), bc2.getEvaluables().size());
            ...
            Assert.assertEquals(msg + ": bags are not equal", bc1.getBag(), bc2.getBag());
        } else {
            Assert.fail(msg + ": non-supported object in Query");
        }
    }

    private static void checkQueryReferences(String msg, QueryReference qr1, QueryReference qr2, Query q1, Query q2) {
        if ((qr1.getQueryClass() != null) || (qr2.getQueryClass() != null)) {
            checkQueryNodes(msg, qr1.getQueryClass(), qr2.getQueryClass(), q1, q2);
        } else {
            Assert.assertEquals(msg + ": collection origin is not equal", ((QueryCollectionReference) qr1).getQcObject(), ((QueryCollectionReference) qr2).getQcObject());
        }
        Assert.assertEquals(msg + ": QueryReference types are not equal", qr1.getType(), qr2.getType());
        Assert.assertEquals(msg + ": QueryReference fieldnames are not equal", qr1.getFieldName(), qr2.getFieldName());
    }
