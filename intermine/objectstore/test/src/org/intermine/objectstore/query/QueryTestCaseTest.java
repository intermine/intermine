...
public class QueryTestCaseTest extends QueryTestCase
{
    ...
    public void testDifferentSubquery() throws Exception {
        ...
        try {
            ...
            assertEquals(e.getMessage(), "asserting equal: expected <" + q1.toString() + "> but was <" + q2.toString() + ">: SELECT lists are not equal: query nodes are not the same: field members of different subquery aliases expected:<a1_> but was:<a2_>", e.getMessage());
        } finally {
            if (failed) {
                fail("Failure should have happened");
            }
