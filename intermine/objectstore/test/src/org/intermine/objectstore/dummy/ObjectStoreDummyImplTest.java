...
public class ObjectStoreDummyImplTest extends OneTimeTestCase
{
    ...
    public void testAddRowRetrieveSame() throws Exception {
        ...
        os.addRow(row);
        os.setResultsSize(1);
        List rows = os.execute(new Query(), 0, 1, true, true, ObjectStore.SEQUENCE_IGNORE);
    ...
    public void testRetrieveNew() throws Exception {
        ...
        os.setResultsSize(1);
        List rows = os.execute(q, 0, 1, true, true, ObjectStore.SEQUENCE_IGNORE);
    ...
    public void testRetrieveNewAfterAdd() throws Exception {
        ...
        os.addRow(row);
        os.setResultsSize(2);
        List rows = os.execute(q, 0, 2, true, true, ObjectStore.SEQUENCE_IGNORE);
    ...
    public void testReachEndOfResults() throws Exception {
        ...
        List rows = os.execute(q, 0, 8, true, true, ObjectStore.SEQUENCE_IGNORE);
        assertEquals(8, rows.size());

        // Try and get the next 7
        rows = os.execute(q, 8, 7, true, true, ObjectStore.SEQUENCE_IGNORE);
        assertEquals(2, rows.size());

        // Try and get rows 10 to 19
        rows = os.execute(q, 10, 10, true, true, ObjectStore.SEQUENCE_IGNORE);
        assertEquals(0, rows.size());

        // Stupidly try and get beyond the end
        rows = os.execute(q, 15, 10, true, true, ObjectStore.SEQUENCE_IGNORE);
        assertEquals(0, rows.size());
    }
    ...
    public void testExecuteCalls() throws Exception {
        ...
        os.setResultsSize(10);
        Results res = os.execute(q);
        os.execute(q, 1, 3, true, true, ObjectStore.SEQUENCE_IGNORE);
        assertEquals(1, os.getExecuteCalls());
        os.execute(q, 5, 2, true, true, ObjectStore.SEQUENCE_IGNORE);
    ...
    public void testPoisonRow() throws Exception {
        ...
        os.setResultsSize(10);
        os.setPoisonRowNo(7);
        os.execute(q, 0, 5, true, true, ObjectStore.SEQUENCE_IGNORE);
        os.execute(q, 8, 2, true, true, ObjectStore.SEQUENCE_IGNORE);
        os.execute(q, 4, 3, true, true, ObjectStore.SEQUENCE_IGNORE);
        try {
            os.execute(q, 0, 10, true, true, ObjectStore.SEQUENCE_IGNORE);
            fail("Expected: ObjectStoreException");
        } catch (ObjectStoreException e) {
        }
        try {
            os.execute(q, 7, 3, true, true, ObjectStore.SEQUENCE_IGNORE);
            fail("Expected: ObjectStoreException");
        } catch (ObjectStoreException e) {
        }
        try {
            os.execute(q, 7, 1, true, true, ObjectStore.SEQUENCE_IGNORE);
            fail("Expected: ObjectStoreException");
        } catch (ObjectStoreException e) {
        }
        try {
            os.execute(q, 0, 8, true, true, ObjectStore.SEQUENCE_IGNORE);
            fail("Expected: ObjectStoreException");
        } catch (ObjectStoreException e) {
        }
    }
    ...
    public void testCount() throws Exception {
        ...
        Query q = new Query();
        assertEquals(objectStore.count(q, ObjectStore.SEQUENCE_IGNORE), 12);
    }
}
