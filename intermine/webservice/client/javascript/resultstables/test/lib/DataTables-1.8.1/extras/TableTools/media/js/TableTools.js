		...
		for ( j=0, jLen=dt.aiDisplay.length ; j<jLen ; j++ )
		{
			...
			for ( i=0, iLen=dt.aoColumns.length ; i<iLen ; i++ )
			{
				if ( aColumnsInc[i] )
				{
                    ...
                    { 
                        var toRender = jQuery.extend(true, {}, dt.aoData[ dt.aiDisplay[j] ]);
                        toRender.aData = toRender._aData;
                        toRender.iDataColumn = i;
                        toRender.iDataRow = j;
                        mTypeData = dt.aoColumns[i].fnRender(toRender);
                    }
					if ( typeof mTypeData == "string" )
					{
						...
						sLoopData =
						 	sLoopData.replace(/<img.*?\s+alt\s*=\s*(?:"([^"]+)"|'([^']+)'|([^\s>]+)).*?>/gi,
						 		'$1$2$3');
                        // Replace links with the displayed value. Discard the location.
                        sLoopData = sLoopData.replace(/<a\s+href\s*=\s*".*"\s*>(.*)<\/a\s*>/gi, '$1');
