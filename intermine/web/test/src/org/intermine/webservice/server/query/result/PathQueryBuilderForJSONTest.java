...
public class PathQueryBuilderForJSONTest extends TestCase {
	...
	public void testDoesntChangeAcceptableViews() {
		...
		pq1a.addViews("Manager.name");
		pathQueries.add(pq1a);
		
		PathQuery pq1b = new PathQuery(model);
		pq1b.addViews("Manager.age", "Manager.name");
		pathQueries.add(pq1b);
		
		PathQuery pq2a = new PathQuery(model);
		pq2a.addViews("Manager.name", "Manager.department.name"); 
		pathQueries.add(pq2a);
		
		PathQuery pq2b = new PathQuery(model);
		pq2b.addViews("Manager.name", "Manager.address.address", "Manager.department.name");
		pathQueries.add(pq2b);
		
		PathQuery pq3a = new PathQuery(model);
		pq3a.addViews("Manager.name", "Manager.department.name", "Manager.department.employees.name");
		pathQueries.add(pq3a);
		
		PathQuery pq3b = new PathQuery(model);
		pq3b.addViews("Company.name", "Company.contractors.seniority", "Company.departments.name");
		pathQueries.add(pq3b);
		
		PathQuery pq4a = new PathQuery(model);
		pq4a.addViews("Company.name", "Company.departments.name", "Company.departments.employees.name", "Company.departments.employees.address.address");
	...
	public void testRearrangesBadlyOrderedViews() {
		...
		pathQueries.add(pq2a);
		expectedViews.add(Arrays.asList("Manager.name", "Manager.department.name")); 
		
		PathQuery pq2b = new PathQuery(model);
		pq2b.addViews("Manager.address.address", "Manager.name", "Manager.department.name");
		pathQueries.add(pq2b);
		expectedViews.add(Arrays.asList("Manager.name", "Manager.address.address", "Manager.department.name"));
		
		PathQuery pq3a = new PathQuery(model);
		pq3a.addViews("Manager.name", "Manager.department.employees.name", "Manager.department.name");
		pathQueries.add(pq3a);
		expectedViews.add(Arrays.asList("Manager.name", "Manager.department.name", "Manager.department.employees.name"));
		
		PathQuery pq3b = new PathQuery(model);
		pq3b.addViews("Company.contractors.seniority", "Company.departments.name", "Company.name");
		pathQueries.add(pq3b);
		expectedViews.add(Arrays.asList("Company.name", "Company.contractors.seniority", "Company.departments.name"));
		...
		pq4a.addViews("Company.departments.employees.name", "Company.departments.name", 
				...
				"Company.name");
		pathQueries.add(pq4a);
		expectedViews.add(Arrays.asList("Company.name", "Company.departments.name", 
