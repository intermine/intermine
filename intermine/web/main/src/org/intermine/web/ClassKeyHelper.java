...
/**
 * Methods to read and manage keys for classes.  Keys define how certain
 * classes are identified and are used in defining bag creation.
 * @author rns
 *
 */
public class ClassKeyHelper 
{
     ...
     * @param props a properties object describing class keys
    ...
    public static Map readKeys(Model model, Properties props) {
        ...
        for (Iterator i = props.entrySet().iterator(); i.hasNext();) {
            ...
            ClassDescriptor cld = model.getClassDescriptorByName(pkg + "." + clsName);
            if (cld != null) {
                ...
                for (int o = 0; o < tokens.length; o++) {
                    ...
                    for (int j = 0; j < keyBits.length; j++) {
                        ...
                        FieldDescriptor fld = cld.getFieldDescriptorByName(keyBit);
                        if (fld != null) {
                            keyFields.add(fld);
                        } else {
                            LOG.warn("problem loading class key: " + keyString + " for class "
                                    + clsName);
                        }
                    }
                    if (keyFields.size() == keyBits.length) {
                        ...
                        while (subIter.hasNext()) {
                            ClassKeyHelper.addKey(classKeys, 
                                    TypeUtil.unqualifiedName(((ClassDescriptor) subIter.next())
                                                             .getName()),
                                    keyFields);
                        }
                    }
     ...
     * @param classKeys existing map of classname to set of keys
     * @param clsName class name for key
     * @param key set of FieldDescriptors that describe the key
     */
     ...
     */
    public static boolean isKeyField(Map classKeys, String clsName, String fieldName) {
     ...
     * @param fieldName the field name to look up
     * @return true if the field is an 'identifying' field for one of the classes that the object is
     */
    public static boolean isKeyField(Map classKeys, InterMineObject o, String fieldName) {
        return getKeyFieldClass(classKeys, o, fieldName) != null;
    }

    /**
     * For a given object/field name combination, if the field is a key field of one of the Class
     * that the object is, return that Class.
     * @param classKeys map of classname to set of keys
     * @param othe object to check
     * @param fieldName the field name to look up
     * @return the Class that fieldName is a key field in, otherwise null
     */
    public static Class getKeyFieldClass(Map classKeys, InterMineObject o, String fieldName) {
