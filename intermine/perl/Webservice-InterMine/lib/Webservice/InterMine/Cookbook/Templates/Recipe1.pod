...
InterMine::Cookbook::Templates::Recipe1 - Accessing Templates

=head1 SYNOPSIS

  use InterMine 'www.flymine.org/query/service';

  my $template = InterMine->template('All_Genes_In_Organism_To_Publications);
...
You can even extent them with roles in exactly the same way:
  ...
  my $template = InterMine->template(
      $name,
      with => [@roles],
  );
...
If I call
  ...
  print InterMine->template($name)->show_constraints;
