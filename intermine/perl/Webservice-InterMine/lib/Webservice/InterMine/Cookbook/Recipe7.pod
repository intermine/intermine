...
InterMine::Cookbook::Recipe7 - Extending InterMine
...
=head1 SYNOPSIS
  ...
  my $query = InterMine->new_query(with => ['WriteOutYaml']);
...
InterMine is designed to incorporate any additions you may want to write in
as simply as possible. Being object orientated, it is possible to subclass and
reimplement the entire InterMine suite. However, a simpler approach is to use
roles(1). The InterMine modules are written using the Moose MOP system, and
...
and then passed to the query constructor:
  ...
  my $query = InterMine->new_query(with => [$role]);

  # note that we can pass a list of roles
  my $query = InterMine->new_query(with => [$role1, $role2]);
...
Combining these two kinds of roles can produce some radically new behaviour:
  ...
  my $query = InterMine->new_query(with => ['HTMLTable']);
...
see examples of roles that can be applied to queries in InterMine/Query/Roles/Extra/
