...
InterMine::Cookbook::Recipe2 - Adding Constraints
...
In the example above we have three different operators: C<CONTAINS>, C<< >= >>,
and C<< <= >>, all of which take a value and use it to constrain their path.

For the simpler constraints, it is also possible to add constraints using the
following pattern:
...
and then you know that this constraint has code 'Q'. Generally however, you shoudln't
