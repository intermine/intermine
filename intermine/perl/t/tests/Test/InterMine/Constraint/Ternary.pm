package Test::InterMine::Constraint::Ternary;

use base ('Test::InterMine::Constraint::Binary');

use Test::More;
use Test::Exception;
use InterMine::Model; # Again, no idea why, but this test class does not compile
                      # without loading this module first?!?
sub class {'InterMine::Constraint::Ternary'};
sub hash {(
    path => 'Some.path.here',
    op   => 'LOOKUP',
    value => 500,
...
sub invalid_operators {
    return ('IS NULL', '=', 'IS IN', '%&$!');
}

sub inheritance : Test(5) {
    my $test   = shift;
    $test->SUPER::inheritance;
    my $parent = 'InterMine::Constraint::Binary';
