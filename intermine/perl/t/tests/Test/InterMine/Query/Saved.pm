package Test::InterMine::Query::Saved;

use base ('Test::InterMine::Query::Core');
use Test::More;
use Test::XML;
use Test::MockObject;
sub class {'InterMine::Query::Saved'}
...
<query name="Query 1" model="testmodel" view="Employee.name Employee.age" constraintLogic="A and B and C">
     ...
     <constraint code="B" op="LIKE" path="Employee.department.name" value="Catering*" />
     <constraint path="Employee.department.company.name" op="IN" code="C" >
...
sub exp_url {q|FAKEROOTFAKEPATH?format=tab&query=%3Cquery+constraintLogic%3D%22A+and+B+and+C%22+model%3D%22testmodel%22+name%3D%22Query+1%22+sortOrder%3D%22Employee.name+asc%22+view%3D%22Employee.name+Employee.age%22%3E%0A++%3Cconstraint+code%3D%22A%22+op%3D%22%3D%22+path%3D%22Employee.age%22+value%3D%2218%22%2F%3E%0A++%3Cconstraint+code%3D%22B%22+op%3D%22LIKE%22+path%3D%22Employee.department.name%22+value%3D%22Catering*%22%2F%3E%0A++%3Cconstraint+code%3D%22C%22+op%3D%22IN%22+path%3D%22Employee.department.company.name%22%3E%0A++++%3Cvalue%3EThe+Sandwich+Van%3C%2Fvalue%3E%0A++++%3Cvalue%3EPr%C3%AAt-%C3%A0-Manger%3C%2Fvalue%3E%0A++++%3Cvalue%3EThe+Cambridge+Blue%3C%2Fvalue%3E%0A++%3C%2Fconstraint%3E%0A%3C%2Fquery%3E%0A%0A|}
