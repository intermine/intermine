package Test::InterMine::Query::Core;
...
sub class {'InterMine::Query::Core'}
...
sub add_constraint : Test(20) {
    ...
    isa_ok($cons[-1], 'InterMine::Constraint::Unary', ".. and it");
    lives_ok(
	sub {
	    push @cons, $obj->add_constraint(
		path => 'Employee.age',
    ...
    isa_ok($cons[-1], 'InterMine::Constraint::Binary', ".. and it");
    lives_ok(
	sub {
	    push @cons, $obj->add_constraint(
    ...
    isa_ok($cons[-1], 'InterMine::Constraint::Ternary', ".. and it");
    lives_ok(
	sub {
	    push @cons, $obj->add_constraint(
    ...
    isa_ok($cons[-1], 'InterMine::Constraint::SubClass', ".. and it");
    lives_ok(
	sub {
	    push @cons, $obj->add_constraint(
		path => 'Employee.title',
    ...
    isa_ok($cons[-1], 'InterMine::Constraint::Multi', ".. and it");
    lives_ok(
	sub {
    ...
    lives_ok(
	sub {
	    push @cons, $obj->add_constraint('Employee.department.company.address.address CONTAINS "UK"');
	},
...
sub sort_order : Test(6) {
    ...
    my @view = ('Employee.name', 'Employee.address.address', 'Employee.department.name');
    $obj->add_view(@view);
    isa_ok($obj->sort_order, 'InterMine::SortOrder', "Constructs a sort order correctly");
...
sub add_pathdescription : Test(3) {
    ...
    isa_ok($obj->path_descriptions->[0], 'InterMine::PathDescription', "And it");
}
