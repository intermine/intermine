package Test::InterMine::ConstraintFactory;
...
sub class { 'InterMine::ConstraintFactory' };
sub types {
    my $test  = shift;
...
sub constraint_types {{
	...
	class => 'InterMine::Constraint::Unary',
    },
    binary => {
	args => [path => 'Some.path', op => '=', value => 500],
	class => 'InterMine::Constraint::Binary',
    },
    ternary => {
	args => [path => 'Some.path', op => 'LOOKUP',
		 value => 500, extra_value => 'NZ'],
	class => 'InterMine::Constraint::Ternary',
    },
    multi => {
	args => [path => 'Some.path', op => 'IN', values => [qw/a b c/]],
	class => 'InterMine::Constraint::Multi',
    },
    subclass => {
	args => [path => 'Some.path', type => 'Some.other.path'],
	class => 'InterMine::Constraint::SubClass',
    },
}}
