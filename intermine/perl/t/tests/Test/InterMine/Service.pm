package Test::InterMine::Service;
...
sub class {'InterMine::Service'}
sub fake_queryurl {'fake.url'}
sub fake_viewlist {[qw/one two three/]}
sub user_agent {'InterMinePerlAPIClient'}
sub object {
    my $test = shift;
    return $test->{object};
};
...
sub setup :Test(setup) {
    ...
    $fakeLWP->mock(env_proxy => sub {})
            ...
            ->mock(get => sub {
			   ...
			   $fakeRes->{_content} = "VERSION_STRING";
		       }
		       else {
	...
	'InterMine::TemplateFactory',
	new => sub {
	    return $fake_TemplateFactory;
	},
    );
    $fake_TemplateFactory->set_isa('InterMine::TemplateFactory');
...
sub _new : Test(6) {
    ...
    is($service->version, "VERSION_STRING",
