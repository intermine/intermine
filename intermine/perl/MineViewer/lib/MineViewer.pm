...
sub get_lists {
    my %lists = map {$_->name => $_} grep {defined} map {$service->list($_)} @$list_names;
...
sub get_items_in_list {
    ...
    my $items = $query->results(as => 'hashrefs');
    return $items;
}
...
get '/list/:list.tsv' => sub {
    ...
    my $query = get_list_query($list);
    content_type 'text/plain';
    header 'Content-Disposition' => 'attachment: filename=' . $list->name . '.tsv';
...
get '/list/:list.json' => sub {
    ...
    header 'Content-Disposition' => 'attachment: filename=' . $list->name . '.json';
...
get '/list/:list.xml' => sub {
    ...
    header 'Content-Disposition' => 'attachment: filename=' . $list->name . '.xml';
...
sub do_item_report {
    ...
    my %contained_in = map {$_->name, $_} $service->lists_with_object($obj->{objectId});
...
post '/add_ids_to_list' => sub {
    ...
    my $list = $service->list($list_name)
        or return to_json({problem => "$list_name does not exist at this service"});
    my $query = $service->new_query(class => $list->type);
    $query->add_constraint('id', 'ONE OF', [@ids]);
    $list += $query;
    return to_json({info => "Added " . scalar(@ids) . " items to $list_name"});
};
