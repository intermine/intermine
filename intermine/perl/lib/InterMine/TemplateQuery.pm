package InterMine::TemplateQuery;

=head1 NAME

InterMine::TemplateQuery - an object representation of a template query

=head1 SYNOPSIS

my $keyword = 'Probe';
my $url = 'path/to/template/service';
my $template_service  = InterMine::WebService::Service::TemplateService->new($url);
my ($err, $templates) = $template_service->search_for($keyword);

for (@$templates) {
    if ($_->name eq "Probe_Gene") {
        my $template = InterMine::Template->new($_);
        my @views    = $template->list_views;
        my @editable_parameters = $template->list_parameters
        for my $p (@editable_parameters) {
            $template->$p($op, $value);
        }
        my ($err, $res) = $template->execute_request;
    }
}
...
You can find documentation for this module with the perldoc command.
    ...
    perldoc InterMine::TemplateQuery
