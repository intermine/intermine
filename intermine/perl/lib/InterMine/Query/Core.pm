...
use InterMine::Join;
use InterMine::PathDescription;
use InterMine::Path qw(:validate);
use InterMine::SortOrder;
use Exporter 'import';
...
sub get_constraint {
    ...
    if (@matches > 1) {
	confess "more than one constraint found - that should never happen. Please report this bug";
    }
    return $matches[0];
}
...
sub coded_constraints {
    my $self = shift;
    my $criterion = sub {$_->does('InterMine::Constraint::Roles::Operator')};
    return $self->find_constraints($criterion);
}

sub sub_class_constraints {
    my $self = shift;
    my $criterion = sub {$_->isa('InterMine::Constraint::SubClass')};
    return $self->find_constraints($criterion);
}
...
sub add_pathdescription {
    my $self = shift;
    my $pd = InterMine::PathDescription->new(@_);
...
sub _build_constraint_factory {
    InterMine::ConstraintFactory->new;
}
   ...
   # (B or G) and (I or F) and J and C and D and E and H and K and L and M and A
	...
	if ($bit =~ /^[\(\)]$/) {
		...
		confess "No constraint with code $bit in this query " .
		    " - we only have " . join(', ', keys %found_con);
	    }
	}
	elsif ($bit =~ /^and$/) {
	    push @processed_bits, ' & ';
...
sub parse_constraint_string {
	...
	m/^(IS\sNOT\sNULL|IS\sNULL|NOT\sIN|\S+)
	  (?:\s+(.*))?
	 /x;
    if (@bits < 1) {
	croak "can't parse constraint: $constraint_string\n";
...
sub validate_paths {
    ...
    my @errs  = map {validate_path($self->model, $_, $self->type_dict)}
                        @paths;
    return @errs;
}
...
sub validate_consistency {
    my $self  = shift;
    my @roots = map {root($self->model, $_, $self->type_dict)} $self->all_paths;
    unless (uniq(@roots) == 1) {
	return "Inconsistent query: all paths must descend from the same root."
               . " - we got: " . join(', ', map {$_->name} uniq @roots) . "\n";
    }
    return undef;
}
