package InterMine::Query::Roles::Templated;
...
sub editable_constraints {
    ...
    my $criterion = sub {
	$_->is_editable and
	    $_->does('InterMine::Constraint::Roles::Operator');
    };
...
sub results_with {
	...
	if (my $con = $clone->get_constraint($code)) {
	    confess "You can only change values and operators for editable constraints" unless $con->is_editable;
	    my %attr = %$con;
	    $attr{value} = $new_value_for{$code} if (exists $new_value_for{$code});
	    $attr{op}    = $new_op_for{$code}    if (exists $new_op_for{$code});
