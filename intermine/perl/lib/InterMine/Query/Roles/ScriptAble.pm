package InterMine::Query::Roles::ScriptAble;
...
sub to_script {
    ...
    $script_buffer .= $NEWLINE x 2
        . join($SPACE, qw(use InterMine), single_quoted($self->service->root))
        . $ENDLINE
        . $NEWLINE x 1;
    $script_buffer .= 'my $query = InterMine->new_query' . $ENDLINE . $NEWLINE x 2;
    $script_buffer .= '$query->add_view('
        . join($COMMA , map( {single_quoted($_)} $self->views) ) . ')'
        . $ENDLINE
        . $NEWLINE x 2;
    for my $elems (qw/joins constraints/) {
        for my $elem ($self->$elems) {
            $script_buffer .= '$query->add_' . substr($elems, 0, -1) .'(';
            while (my ($key, $value) = each %$elem) {
                $script_buffer .= $TAB . $key . ' => ' . single_quoted($value) . $COMMA;
            }
            $script_buffer .= ')' . $ENDLINE;
        }
        $script_buffer .= $NEWLINE;
    }
    $script_buffer .= $NEWLINE . 'my $results = $query->results' . $ENDLINE;
