package InterMine::Query::TemplateHandler;
...
override start_element => sub {
	...
	$self->query->description($args->{Attributes}{longDescription})
...
override process_constraint_attr => sub {
    ...
    if ($attr->{switchable}) {
	$temp_args{is_locked}   = ($attr->{switchable} eq 'locked') ? 1 : 0;
	$temp_args{switched_on} = ($attr->{switchable} eq 'off') ? 0 : 1;
    }
