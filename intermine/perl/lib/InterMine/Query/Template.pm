package InterMine::Query::Template;

use Moose;
use InterMine::Query::TemplateHandler;
...
sub _build_handler {
    InterMine::Query::TemplateHandler->new;
}

sub _build_constraint_factory {
    InterMine::TemplateConstraintFactory->new;
}
...
InterMine::Query::Template - A representation of a webservice template

=head1 SYNOPSIS

  use InterMine 'www.flymine.org/query/service';

  my $template = InterMine->template('Probe_Genes');
...
Generally they have all the same methods as queries do (see L<InterMine::Query>),
