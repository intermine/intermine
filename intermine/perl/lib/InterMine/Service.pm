...
InterMine::Service - an object representation of an InterMine Web-Service

=head1 SYNOPSIS

  use InterMine;

  my $service = InterMine->get_service('www.flymine.org/query/service');
...
use InterMine::TypeLibrary qw(Uri Model TemplateFactory SavedQueryFactory ListFactory);
...
use constant {
    ...
    USER_AGENT         => 'InterMinePerlAPIClient',
    LIST_PATH          => '/lists/xml',
...
has _saved_queries => (
    ...
    handles => {
	saved_query => 'get_saved_query_by_name',
    },
);
has _lists => (
    is => 'ro',
...
sub get_results_iterator {
    ...
    my $response = InterMine::ResultIterator->new(
