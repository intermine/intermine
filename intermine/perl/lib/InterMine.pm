...
InterMine - modules for interacting with InterMine datawarehouse webservices

=head1 SYNOPSIS

    use InterMine;

    my $service  = InterMine->new_service($url);
...
You can call C<use InterMine> without any parameters, which simply
means you need to either specify the webservice url on every call, or
call the methods on a service directly.

If you call C<use InterMine $url>, a default service will be set,
...
Please see L<InterMine::Query::Template>
...
Please see L<InterMine::Query::Saved>
...
Please see: L<InterMine::Service>
...
sub get_service {
	...
	my $service = InterMine::Service->new($url);#, $user, $pass);
...
=item * L<InterMine::Cookbook> for guide on how to use these modules.

=item * L<InterMine::Query>

=item * L<InterMine::Service>

=item * L<InterMine::Query::Template>

=item * L<InterMine::Query::Saved>
