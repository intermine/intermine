...
from intermine.webservice import JSONIterator, EchoParser
...
class List(object):
    ...
    def calculate_enrichment(self, widget, background = None, correction = "Holm-Bonferroni", maxp = 0.05, filter = None):
        """Perform an enrichment calculation on this list"""
        params = dict(list = self.name, population = background, widget = widget, correction = correction, maxp = maxp, filter = filter)
        form = urllib.urlencode(params)
        uri = self._service.root + self._service.LIST_ENRICHMENT_PATH
        resp = self._service.opener.open(uri, form)
        return JSONIterator(resp, EchoParser())
