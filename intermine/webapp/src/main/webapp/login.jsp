      ...
      <h3 class="oauth"><fmt:message key="login.oauth2"/></h3>
        <c:forEach var="provider" items="${OAUTH2_PROVIDERS}">
          <c:choose>
            <c:when test="${fn:toLowerCase(provider) == 'elixir'}">
              <a class="elixir" href="/${WEB_PROPERTIES['webapp.path']}/oauth2authenticator.do?provider=${provider}">
                <img src="images/elixir-login.png" width="22" height="22" alt="Log in with ELIXIR"/>
              </a>
           </c:when>
          ...
          <c:when test="${fn:toLowerCase(provider) == 'im'}">
            <a href="/${WEB_PROPERTIES['webapp.path']}/oauth2authenticator.do?provider=${provider}">
              <img class="icon-oauth" src="icons/intermine32x32.png" width="22" height="22" alt="Log in with InterMine" />
                IM
            </a>
         </c:when>
          <c:otherwise>
