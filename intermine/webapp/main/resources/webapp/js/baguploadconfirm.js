...
function addId2Bag(objectId, row, parentId, issueType) {
    ...
    // Switch on highlight.
    jQuery('table.type-all-'+issueType + ' tbody tr:nth-child('+row+') td').addClass('highlight');
...
function removeIdFromBag(objectId, row, parentId, issueType) {
    ...
    jQuery('table.type-all-'+issueType + ' tbody tr:nth-child('+row+') td').removeClass('highlight');
...
function addAll(issue, flatArray){
    ...
    jQuery.each(flatArray.split("|"), function(i, obj) {
        var parts = obj.split(',');
        types[parts[3]] = true; // add the type
        addIdToListOfMatches(parts[0]); // actually add
    });

    // Update the UI for each class.
    for (type in types) {
        jQuery('table.type-all-'+type + ' tbody td').addClass('highlight');
    }
...
function removeAll(issue, flatArray){
    ...
    jQuery.each(flatArray.split("|"), function(i, obj) {
        var parts = obj.split(',');
        types[parts[3]] = true; // add the type
        removeIdFromListOfMatches(parts[0]); // actually remove
    });

    // Update the UI for each class.
    for (type in types) {
        jQuery('table.type-all-'+type + ' tbody td').removeClass('highlight');
    }
