			...
			/*if(params.node.edgeCircleHighlight){
				...
				var circleObject = params.svg.draw(
					new Smits.PhyloCanvas.Render.Circle(
						params.x, params.y, 5,
						{ attr: {fill: Smits.PhyloCanvas.Render.Style.highlightedEdgeCircle.fill}}
					)
				);
...
Smits.PhyloCanvas.Render.SVG.prototype = {
	...
	draw : function(instruct){
	   ...
	   if(instruct.type == 'line'){
			...
			obj = this.svg.circle(instruct.x, instruct.y, instruct.radius, instruct.attr);
			/*.attr({
				"stroke": 'red'
			});*/
	...
	renderBinaryChart = function(x, groupName, params){
		...
		for(var i = 0; i < labelsHold.length; i++){
			...
			if (node.chart[groupName] && node.chart.text) {
			    var attr = {fill: Smits.PhyloCanvas.Render.Style.getStyle(node.chart[groupName], 'textSecantBg').fg};
    			if (node.chart.uri) { attr.href = node.chart.uri };
    			var text = svg.draw(new Smits.PhyloCanvas.Render.Text(
    			    x + bufferInner + 5, node.y, 
    			    node.chart.text,
			    	{
						attr: attr
					}
