...
<style>
  #cufflinks-expression div.data-table { margin-top:20px; }
</style>

<div id="cufflinks-expression">

<c:choose>
  <c:when test="${empty(cufflinksResults)}">
    <h3 class="goog gray">Expression by Experiment</h3>
    <p>No expression data available for this gene.</p>
  </c:when>
  <c:otherwise>
    <h3 class="goog">Expression by Experiment</h3>
    <div class="chart" id="cufflinks-expression-chart"></div>
    ...
    <div class="data-table collection-table" style="display:none;">
      <h3>Table</h3>
      <c:set var="inlineResultsTable" value="${cufflinksCollection}" />
      <tiles:insert page="/reportCollectionTable.jsp">
        ...
        <tiles:put name="fieldName" value="cufflinksExpression" />
      </tiles:insert>
    </div>
    ...
    <script type="text/javascript">
      function drawChart() {
          ...
          var n = 0;
          <c:forEach var="stage" items="${cufflinksResults}">
            data.addRows(1);
          ...
          <%-- aim & shoot --%>
          var chart = new google.visualization.BarChart(document.getElementById("cufflinks-expression-chart"));
      ...
      <%-- toggle table --%>
      jQuery("#cufflinks-expression input.toggle").click(function() {
        jQuery("#cufflinks-expression div.collection-table").toggle();
      });
    </script>
