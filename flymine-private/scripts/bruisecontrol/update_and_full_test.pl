...
{
  open my $svn_status, "svn status -u |" or die "failed to open pipe to svn: $!\n";

  while (my $stat_line = <$svn_status>) {
    if ($stat_line =~ /^\s+\*\s+\d+/) {
      $updates_available = 1;
    }
  }
...
{
  open my $svn_log, "svn log --revision HEAD|" or die "can't open pipe to svn: $!\n";
