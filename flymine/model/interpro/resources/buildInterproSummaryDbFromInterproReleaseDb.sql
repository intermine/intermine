drop database interpro_prod;

create database interpro_prod;
...
create table interpro_prod.cv_entry_type as select code, abbrev, description from interpro10.cv_entry_type limit 10000000;
alter table interpro_prod.cv_entry_type add column cv_entry_type_id INTEGER(32) AUTO_INCREMENT primary key first;

create table interpro_prod.cv_relation as select code, abbrev, description, forward, reverse from interpro10.cv_relation limit 10000000;
alter table interpro_prod.cv_relation add column cv_relation_id INTEGER(32) AUTO_INCREMENT primary key first;

create table interpro_prod.cv_evidence as select code, abbrev, description from interpro10.cv_evidence limit 10000000;
alter table interpro_prod.cv_evidence add column cv_evidence_id INTEGER(32) AUTO_INCREMENT primary key first;

create table interpro_prod.cv_database as select dbcode, dbname, dborder, dbshort from interpro10.cv_database limit 10000000;
alter table interpro_prod.cv_database add column cv_database_id INTEGER(32) AUTO_INCREMENT primary key first;

create table interpro_prod.db_version as select dbcode, version, entry_count, file_date, load_date from interpro10.db_version where dbcode in (select dbcode from interpro_prod.cv_database) limit 10000000;
...
create table interpro_prod.taxonomy as select DISTINCT tax_id as taxa_id from interpro10.taxonomy2protein where tax_id in (6239,7165,7227,180454);
alter table interpro_prod.taxonomy add column taxonomy_id INTEGER(32) AUTO_INCREMENT primary key first;

create table interpro_prod.protein2taxonomy as select protein_ac, tax_id, hierarchy, tax_name_concat from interpro10.taxonomy2protein where tax_id in (select taxa_id from interpro_prod.taxonomy);
...
create table interpro_prod.protein as select ip10.protein_ac, ip10.name, ip10.dbcode, ip10.crc64, ip10.len, ip10.fragment, ip10.struct_flag from interpro10.protein ip10, interpro_prod.protein2taxonomy itp2t where ip10.protein_ac = itp2t.protein_ac;
...
create table interpro_prod.method as select method_ac, name, dbcode, method_date, skip_flag from interpro10.method limit 10000000;
...
create table interpro_prod.entry as select entry_ac, entry_type, name, checked, created, short_name from interpro10.entry limit 10000000;
...
create table interpro_prod.supermatch as select i10s.protein_ac, i10s.entry_ac, i10s.pos_from, i10s.pos_to from interpro10.supermatch i10s, interpro_prod.protein itp where i10s.protein_ac = itp.protein_ac;
...
create table interpro_prod.matches as select i10m.protein_ac, i10m.method_ac, i10m.pos_from, i10m.pos_to, i10m.status, i10m.dbcode, i10m.evidence, i10m.seq_date, i10m.match_date, i10m.score from interpro10.matches i10m, interpro_prod.protein itp where i10m.protein_ac = itp.protein_ac;
...
create table interpro_prod.common_annotation as select ann_id, name, text, comments from interpro10.common_annotation limit 10000000;
alter table interpro_prod.common_annotation add column common_annotation_id INTEGER(32) AUTO_INCREMENT primary key first;


create table interpro_prod.entry2common_annotation as select entry_ac, ann_id, order_in from interpro10.entry2common limit 10000000;
...
create table interpro_prod.entry2cv_database as select entry_ac, dbcode, ac, name from interpro10.entry_xref limit 10000000;
...
create table interpro_prod.entry2method as select entry_ac, method_ac, evidence, ida from interpro10.entry2method limit 10000000;
...
create table interpro_prod.entry2comp as select entry1_ac, entry2_ac, relation from interpro10.entry2comp limit 10000000;
...
create table interpro_prod.entry2entry as select entry_ac, parent_ac, relation from interpro10.entry2entry limit 10000000;
