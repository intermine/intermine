...
public class InterproDataTranslator extends DataTranslator
{
   ...
   * */
  public InterproDataTranslator(
          ItemReader itemReader, Properties properties, Model sourceModel, Model targetModel) {
  ...
  private void processProteinItem(
    ...
    java.util.List matchesList = getItemsViaItemPath(srcItem, PROTEIN_FROM_MATCHES, srcItemReader);
  ...
  private void processMethodItem(
          org.intermine.xml.full.Item srcItem, org.intermine.xml.full.Item tgtItem)
          throws ObjectStoreException {

    //NOTE: This extended path only work as the bridge table has a 1 to 1 row count with the method
    // table.
    //HERE WE WANT TO ESTABLISH A LINK BETWEEN THE METHOD AND ITS RELATED INTERPRO_ID FROM THE ENTRY
    // TABLE
    ...
    java.util.List matchesList = getItemsViaItemPath(srcItem, METHOD_FROM_MATCHES, srcItemReader);
  ...
  private void processEntryItem(
    ...
    if (tgtItem.hasCollection(COMMENTS)) {
      ...
      java.util.List entryToCommmonAnnotationList =
              getItemsViaItemPath(srcItem, ENTRY_FROM_ENTRY_TO_COMMON_ANNOTATION, srcItemReader);
    ...
    setupEntryRelations(ENTRY_FROM_ENTRY_TO_COMP_VIA_ENTRY_ONE, srcItem, tgtItem, ENTRY2, CONTAINS);
    setupEntryRelations(ENTRY_FROM_ENTRY_TO_COMP_VIA_ENTRY_TWO, srcItem, tgtItem, ENTRY1, FOUNDIN);
  }
      ...
      for (Iterator familyIterator = familyList.iterator(); familyIterator.hasNext();) {
        ...
        if (relationshipItem.hasReference(relationToMapTo)) {
          ...
          org.intermine.xml.full.Item nextRelative =
                  ItemHelper.convert(this.srcItemReader.getItemById(relativeRef.getRefId()));
   ...
   * am dealing with a protein via the supermatch table and need to make sure the entry_type is set
   * in the entry that the protein refers to so we can classify it as either a family or domain and
   * put into the correct collection for each protein...
   * <p/>
   *
  ...
  public static Map getPrefetchDescriptors() {
...

    {
      ...
      p2m2mDesc.addConstraint(new ItemPrefetchConstraintDynamic(
              "method", ObjectStoreItemPathFollowingImpl.IDENTIFIER));
    ...
    }
    {
      ...
      m2e2eDesc.addConstraint(new ItemPrefetchConstraintDynamic(
              ObjectStoreItemPathFollowingImpl.IDENTIFIER, "method"));
      m2e2eDesc.addConstraint(new FieldNameAndValue(ObjectStoreItemPathFollowingImpl.CLASSNAME,
  ...
  private static Set getEntryPrefetchDescriptorSet() {
      ...
      eToe2eViaEDesc.addConstraint(new ItemPrefetchConstraintDynamic(
              ObjectStoreItemPathFollowingImpl.IDENTIFIER, "entry"));
      eToe2eViaEDesc.addConstraint(new FieldNameAndValue(ObjectStoreItemPathFollowingImpl.CLASSNAME,
      ...
      eToe2eViaPDesc.addConstraint(new FieldNameAndValue(ObjectStoreItemPathFollowingImpl.CLASSNAME,
              "http://www.flymine.org/model/interpro#entry2entry", false));
