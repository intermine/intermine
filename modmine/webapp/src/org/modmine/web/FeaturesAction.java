...
public class FeaturesAction extends InterMineAction
{
    ...
    throws Exception {
        ...
        if (type.equals("experiment")) {
            ...
            List<String> notLocated = new ArrayList<String>();

            LOG.info("HERE THEY ARE: EXP " + expSubsIds + "<-");

            for (String subId : expSubsIds){
                
                Integer id = Integer.valueOf(subId); 
                LOG.info("HERE THEY ARE: FOR " + subId + "<->" + id +"|");
                
                if (MetadataCache.getUnlocatedFeatureTypes(os).containsKey(new Integer(subId))){
//                if (MetadataCache.getUnlocatedFeatureTypes(os).get(subId) != null)
                    LOG.info("HERE THEY ARE: LIST " + subId + "<-" );
                
                    notLocated.addAll(MetadataCache.getUnlocatedFeatureTypes(os).get(new Integer(subId)));
                }
            }
            
            Set allUnlocated = 
                new HashSet(notLocated);
            
            LOG.info("HERE THEY ARE: UNL " + allUnlocated.toString()+ "<-");
            
            
//            Set<String> subWithNotLocatedFeats = MetadataCache.getUnlocatedFeatureTypes(os).keySet();
//            
//            Set allExpSubs = 
//                new HashSet(expSubsIds);
//            
//            Set<String> intersection = new HashSet<String>(allExpSubs);
//            intersection.retainAll(subWithNotLocatedFeats);
//
//            if (!intersection.isEmpty()){
//             Set unLocatedFTypes =   MetadataCache.getUnlocatedFeatureTypes(os).entrySet() 
//            }
...
//            Map <Integer, String> QQ = MetadataCache.getLocatedFeatureTypes(os);
//            String  locFeature = MetadataCache.getLocatedFeatureTypes(os).get(dccId);
//            int dccNr = new Integer(dccId).intValue();
//            String qq = QQ.get(dccNr);

            List<String>  locFeatures = MetadataCache.getLocatedFeatureTypes(os).get(new Integer(dccId));

//          Map <Integer, String> QQ = MetadataCache.getLocatedFeatureTypes(os);
//          String  locFeature = MetadataCache.getLocatedFeatureTypes(os).get(ddccId);

            //            LOG.info("HERE THEY ARE: " +QQ + "|" +qq + "<-");
            
            LOG.info("HERE THEY ARE: " + dccId + "|" +locFeatures + "<-");
            
            if (locFeatures.contains(featureType)){
                q.addView(featureType + ".primaryIdentifier");
                q.addView(featureType + ".score");
                q.addView(featureType + ".chromosome.primaryIdentifier");
                q.addView(featureType + ".chromosomeLocation.start");
