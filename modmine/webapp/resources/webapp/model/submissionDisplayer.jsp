...
<%--
  <div id="submissionRepositories">
      <c:forEach items="${object.databaseRecords}" var="db" varStatus="db_status" begin="1" end="4">
      <c:choose>
      <c:when test="${db_status.count < 4}" >
        <b>${db.database }:</b> <html:link href="${db.url}">${db.accession}</html:link>      
      </c:when>
      <c:otherwise>
      ....<br></br>
 <im:querylink text="All entries in public repositories generated by this submission" skipBuilder="true" title="View all entries in public repositories (AE, GEO, SRA,..) generated by this submission">
  ...
  <node path="Submission.DCCid" type="Integer">
    <constraint op="=" value="${object.dCCid}" description="" identifier="" code="A">
    </constraint>
  </node>
</query>
     </im:querylink>
        ...
        <c:if test="${db_status.count < 3 }"> <br></c:if>
      </c:forEach>
  </div>
  --%>
