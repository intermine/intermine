...
<div class="body">
	...
	<html:link linkName="#" styleId="sis" style="cursor:pointer">
		<h3>
			Protocols used for this submission (click to toggle) <img
				src="images/undisclosed.gif" id="co">
		</h3>
	</html:link>
		...
		<table width="100%" cellpadding="0" cellspacing="0" border="0"
			...
			<c:forEach items="${protocols}" var="prot" varStatus="p_status">
				...
				<tr class="<c:out value="${pRowClass}"/>">
					...
					<td><a href="${prot.wikiLink}" class="value extlink"></a></td>
					<td class="description">
						<div class="tbox">
							<doopen> <img src="images/undisclosed.gif"> <i>${fn:substring(prot.description,0,80)}...
							</i> </doopen>
							<doclose> <img src="images/disclosed.gif"> <i>${prot.description}</i>
							</doclose>
						</div>
					</td>
				</tr>
	...
	<c:choose>
		<c:when test="${fn:length(pagedResults.rows) >= 1}">
			<table cellspacing="0" width="100%">
				<tr>
					<TD colspan=2 align="left" style="padding-bottom: 10px"><c:set
							var="dccNumber" value="${fn:substringAfter(DCCid,'modENCODE_')}" />
						<c:set var="geoUrl"
							value="http://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=" /> <c:set
							var="srrUrl"
							value="http://www.ncbi.nlm.nih.gov/sites/entrez?db=sra&report=full&term=" />

						<html:link linkName="#" styleId="bro" style="cursor:pointer">
							<h3>
								Browse metadata for this submission (click to toggle)<img
									src="images/undisclosed.gif" id="oc">
							</h3>
						</html:link>
							...
							<table cellpadding="0" cellspacing="0" border="0" class="results">
								...
								<tbody>
									<c:forEach var="row" items="${pagedResults.rows}"
										...
										<c:forEach var="subRow" items="${row}"
											varStatus="multiRowStatus">
											...
											<c:if test="${isFirstValue == 'true'}">
												<c:set var="step" value="${subRow[0].value.field}"
													scope="request" />
											</c:if>
											<c:set var="stepClass">
												<c:choose>
											...
											<tr class="<c:out value="${stepClass}${rowClass}"/>">
												...
												<c:forEach var="column" items="${pagedResults.columns}"
													varStatus="status2">
													<im:instanceof instanceofObject="${subRow[column.index]}"
														instanceofClass="org.intermine.api.results.flatouterjoins.MultiRowFirstValue"
														instanceofVariable="isFirstValue" />
													<c:if test="${isFirstValue == 'true'}">
														<c:set var="resultElement"
															value="${subRow[column.index].value}" scope="request" />
														<c:choose>
															...
															<c:when test="${column.index == 1  || column.index == 5}">
																<c:if
																	test="${fn:startsWith(resultElement.field,'Anonymous Datum')}">
																	<td colspan="2"
																		rowspan="${subRow[column.index].rowspan}"><c:choose>
																			<c:when test="${column.index == 1}">
																				<i><c:out
																						value="output from step ${prevStep -1} -->" /></i>
																			</c:when>
																			<c:otherwise>
																...
																<c:if
																	test="${fn:length(fn:substringBefore(resultElement.field,'File')) gt 0}">
																	<c:set var="output" value="true" />
															...
															<c:otherwise>
																...
																<c:if test="${output}">
																	<td
																		...
																		<c:choose>
																			<c:when
																				test="${fn:startsWith(fn:trim(resultElement.field), 'http://') || fn:startsWith(fn:trim(resultElement.field), 'ftp://')}">
																				<a href="${resultElement.field}"
																					class="value extlink"> <c:set var="elements"
																						value="${fn:split(resultElement.field,'/')}" /> <c:out
																						value="${elements[fn:length(elements) - 1]}" />
																				</a>
																			</c:when>
																			<c:when
																				test="${fn:startsWith(fn:trim(resultElement.field), 'GSM')}">
																				<a href="${geoUrl}${resultElement.field}"
																					class="value extlink"> <c:out
																						value="${resultElement.field}" />
																			</c:when>
																			<c:when
																				test="${fn:startsWith(fn:trim(resultElement.field), 'SRR')}">
																				<a href="${srrUrl}${resultElement.field}"
																					class="value extlink"> <c:out
																						value="${resultElement.field}" />
																			</c:when>
																			<c:when test="${isFile}">
																			...
																			<c:when test="${doLink}">
																				<c:forEach items="${files}" var="file"
																					varStatus="f_status">
																					<c:if
																						test="${fn:contains(resultElement.field, file.name)}">
																						<c:set var="url" value="${file.url}" />
																					</c:if>
																				</c:forEach>
																				<a href="${url}"
																					title="Download file ${resultElement.field}"
																					class="value extlink"> <c:out
																						value="${resultElement.field}" />
																				</a>
																				<%-- </td> --%>
		...
		<c:otherwise>
			...
			<im:querylink
				text="<h3>Browse metadata for this submission (click to view)</h3>"
				skipBuilder="true">
				<%--<im:querylink text="${nrSubs} submissions " skipBuilder="true">--%>
				<query name="" model="genomic"
					view="AppliedProtocol.step AppliedProtocol:inputs.name AppliedProtocol:protocol.name AppliedProtocol:inputs.value AppliedProtocol:outputs.name AppliedProtocol:outputs.value"
					sortOrder="AppliedProtocol.step asc"> <node
					path="AppliedProtocol" type="AppliedProtocol"> </node> <node
					path="AppliedProtocol.submission" type="Submission"> </node> <node
					path="AppliedProtocol.submission.DCCid" type="String"> <constraint
					op="=" value="${DCCid}" description="" identifier="" code="A"></constraint>
				</node> </query>
			</im:querylink>
		</c:otherwise>
	</c:choose>
